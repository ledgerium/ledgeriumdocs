...
.. _transaction-gas-limit:
   ...
   light client
      A client program that allows users in low-capacity environments to still be able to execute and check the execution of transactions without needing to run a full Ledgerium node (Geth).
   ...
   coinbase
      Coinbase is analogous to XLGbase, but is a more generic term for all cryptocurrency platforms.
   ...
   LVM
      Ledgerium Virtual Machine, the decentralized computing platform which forms the core of the Ledgerium platform.
   ...
   testnet
      A mirror network of the production Ledgerium network that is meant for testing.
   ...
   network id
      A number which identifies a particular version of the Ledgerium network.
   ...
   base layer service
      Services such as SWARM and Whisper which are built into the Ledgerium platform.
   ...
   geth
      Ledgerium client implemented in the Golang programming language, based on the protocol as defined in the Ledgerium Yellow Paper.

   parity
      Ledgerium client implemented in the Rust programming language, based on the protocol as defined in the Ledgerium Yellow Paper.

   difficulty
      In very general terms, the amount of effort required to mine a new block. With the launch of Homestead, the `difficulty adjustment algorithm will change <https://github.com/Ledgerium/EIPs/blob/master/EIPS/eip-2.md>`_.
